...
function FrontEndData(props) {
  ...
  const setOptParam = () => {
    ...
    if (bandIsNumber) {
      ...
      defValue.current = componentsOptions.filter(
        (element) => element.value === band.current
      )[0];
    } else {
      optParam.current = "";
  ...
  const componentsList = [
    {
      ...
      url: () => {
        return (
          ...
          "/front_end/components/list?config=" +
          config.current +
          optParam.current
        );
      },
    ...
    },
    {
      ...
      url: () => {
        return (
          process.env.REACT_APP_DB_HOSTNAME +
          "/test_data/noise_temperature/list?config=" +
          config.current +
          optParam.current
        );
      },
    ...
    },
    {
      ...
      url: () => {
        return (
          process.env.REACT_APP_DB_HOSTNAME +
          "/test_data/amplitude_stability/list?config=" +
          config.current +
          optParam.current
        );
      },
    ...
    },
    {
      ...
      url: () => {
        return (
          process.env.REACT_APP_DB_HOSTNAME +
          "/test_data/beam_pattern/list?config=" +
          config.current +
          optParam.current
        );
      },
    ...
    },
    {
      ...
      url: () => {
        return (
          process.env.REACT_APP_DB_HOSTNAME +
          "/test_data/lo_locking/list?config=" +
          config.current +
          optParam.current
        );
      },
    ...
    },
    {
      ...
      url: () => {
        return (
          process.env.REACT_APP_DB_HOSTNAME +
          "/test_data/if_spectrum/list?config=" +
          config.current +
          optParam.current
        );
      },
  ...
  const fetchData = async (elementType) => {
    await axios.get(elementType.url()).then(
      (result) => {
  ...
  mainComponent.current = componentsList.map((obj) => {
    if (obj.show[0]) {
      ...
      if (obj.isLoaded[0] && obj.items[0] != null) {
        ...
        componentTemp = (
          <Row className="p-0 loader-box w-100">
            <NraoLoader speed={3} />
          </Row>
        );
      }
      return (
        <div key={obj.label}>
          <Row>
            {" "}
            <span className="list-title">{obj.label}</span>
            <br />
          </Row>
  ...
  return (
    <Container fluid className="w-100">
      <Row className="w-100">
        ...
        <Col xs={{ span: 8 }}>
          <Route path={`${path}`}>{mainComponent.current}</Route>
        </Col>
      </Row>
    </Container>
  );
