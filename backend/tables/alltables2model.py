...
def table2model(table_name):
    ...
    model_file.write("from app.models.basemodel import *\n\n\n")
    ...
    with open(table_name) as csvfile:
        ...
        for row in reader:
            ...
            if "int" in row[1]:
                model_file.write(
                    "    " + row[0] + " = " + "IntegerField(" + column_name + ")\n"
                )
            elif "double" in row[1]:
                model_file.write(
                    "    " + row[0] + " = " + "DoubleField(" + column_name + ")\n"
                )
            elif "float" in row[1]:
                model_file.write(
                    "    " + row[0] + " = " + "FloatField(" + column_name + ")\n"
                )
            elif "timestamp" in row[1]:
                model_file.write(
                    "    " + row[0] + " = " + "TimestampField(" + column_name + ")\n"
                )
            elif "char" in row[1]:
                model_file.write(
                    "    " + row[0] + " = " + "TextField(" + column_name + ")\n"
                )
            elif "text" in row[1]:
                model_file.write(
                    "    " + row[0] + " = " + "TextField(" + column_name + ")\n"
        ...
        if len(keys) == 0:
            ...
            model_file.write("        primary_key = CompositeKey(" + composite + ")")
        model_file.write("\n")
