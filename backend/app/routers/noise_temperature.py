...
routerNoiseTemperature = APIRouter(
    prefix="/noise_temperature",
    tags=["noise_temperature"]
)
...
async def getNoiseTemperatureResults(keyheader: int, temp: Optional[bool] = True):
    ...
    for noise_temp in query:
        ...
        if temp:
            ...
            max_y = max(max_y, noise_temp.Pol0Sb1YFactor, noise_temp.Pol0Sb2YFactor,
                        noise_temp.Pol1Sb1YFactor, noise_temp.Pol1Sb2YFactor)
            if band in [9, 10]:
                item = {
                    "FreqLO": noise_temp.FreqLO,
                    "CenterIF": noise_temp.CenterIF,
