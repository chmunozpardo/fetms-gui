...
async def getHealthCheckHeaders(FE_Config: int, band: int, Data_Status: int, Data_Type: str):
    result = []
...
async def getHealthCheckResults(FE_Config: int, band: int, Data_Status: int, Data_Type: str):
    result = []
    dataType = dataTypeMapping.get(Data_Type, "Invalid Option")
    ...
    TD = TestData_header.alias()
    query = CCA.select() \
        .where(CCA.fkHeader.in_(
               TD.select(TD.keyId)
               .where((TD.fkFE_Config == FE_Config) &
                      (TD.fkTestData_Type == dataType) &
                      (TD.Band == band) &
                      (TD.fkDataStatus == Data_Status))
               ))
