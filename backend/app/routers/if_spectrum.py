...
routerIFSpectrum = APIRouter(prefix="/if_spectrum", tags=["if_spectrum"])
...
async def getIFSpectrumResults(keyheader: int):
    ...
    query_subheader = IF_SH.select().where(
        (IF_SH.fkHeader == keyheader) & (IF_SH.IFGain == 15)
    )

    min_power = sys.float_info.max
    max_power = -sys.float_info.max
    for if_subheader in query_subheader:
        query = IF.select().where((IF.fkSubHeader == if_subheader.keyId))
        data.append([])
        ...
        for if_spectrum in query[10:]:
            ...
            data[-1].append(
                {"Freq": if_spectrum.Freq_Hz / 1e9, "Power": if_spectrum.Power_dBm}
            )
    result = {
